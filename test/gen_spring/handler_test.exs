defmodule GenSpring.HandlerTest do
  ...
  describe "OTP" do
    test "implements the :sys behavior" do
      ...
      defmodule server_module do
        ...
        @impl GenSpring
        def init(_buffer, _opts), do: {:noreply, %{initial: :state}}
      ...
      server = start_supervised!({GenSpring.Handler, buffer: self(), server: {server_module, []}})
  ...
  describe "initialization" do
    ...
    #   start_result =
    #     start_supervised(
    #       {GenSpring.Handler, buffer: self(), server: {server_module, module_opts}}
    #     )
    #
  ...
  describe "" do
    defmodule ReceivesRequests do
      use GenSpring.Handler

      def init(_buffer, _opts), do: {:noreply, %{requests: []}}
    ...
    test "" do
      {:ok, server} = GenSpring.Handler.start_link(buffer: self(), server: {ReceivesRequests, []})
